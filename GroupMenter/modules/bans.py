...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Bring an order from United association to fight a Demon disaster."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from United association to fight a Tiger disaster."
            )
...
__help__ = """
 ❍ /thiscommandsecret*:* punchs the user who issued the command
